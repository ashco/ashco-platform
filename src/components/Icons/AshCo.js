...
const AshCo = ({ theme }) => {
  ...
  const aAnimation = useSpring({
    transform: 'translate3d(0px, 0px, 0)', from: { transform: 'translate3d(0px, 5000px, 0)' }
  });
  const cAnimation = useSpring({
    transform: 'rotate(-360deg)', from: { transform: 'rotate(0deg)' }
  });
  const oAnimation = useSpring({
    transform: 'rotate(360deg)', from: { transform: 'rotate(0deg)' }
  });
  const ioAnimation = useSpring({
    transform: 'scale(1)', from: { transform: 'scale(0.1)' }
  });
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <g transform="translate(487.000000, 374.000000)">
            <animated.path
